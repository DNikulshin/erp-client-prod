  ...
  const currentItems = listItems
    .sort((a, b) => {
      if (a?.date?.todo && b?.date?.todo) {
        return a?.date?.todo > b?.date?.todo ? 1 : a?.date?.todo < b?.date?.todo ? -1 : 0
      }
      return 0
    })
    .filter((item) => {
    ...
    <>
      {!loading && countItems > 0 && (
        <div className="bg-light p-2 text-center">
          ...
          {selectedOption.value === 'Выполнено' && currentItems.length > 0 && (
            <span>
              Выполнено: <span className="text-success">{currentItems.length}</span>
            </span>
          )}
        </div>
      )}
      ...
      <ReactPaginate
        ...
        nextLabel={
          <svg
            ...
            height="32"
            fill="currentColor"
            className="bi bi-arrow-right-circle btn-hover"
            viewBox="0 0 16 16"
          >
            <path
              fillRule="evenodd"
        ...
        previousLabel={
          <svg
            ...
            height="32"
            fill="currentColor"
            className="bi bi-arrow-left-circle btn-hover"
            viewBox="0 0 16 16"
          >
            <path
              fillRule="evenodd"
